...
public class CoimbraRegionWPTandSyntheticPopulation {
public static void main(String[] args) throws IOException, ParseException {
		
		Config config = ConfigUtils.createConfig();
//		config.global().setNumberOfThreads(6);
		
//		File f = Paths.get("data", "population", "test_synthetic.tsv").toFile();
...
//		config.plans().setInputFile("scenarios/coimbraRegion/synthetic_population.xml");
		config.plans().setInputFile("scenarios/coimbraRegion/population.xml");
//		config.facilities().setInputFile("scenarios/coimbraRegion/synthetic_facilities.xml");
		...
		config.controler().setOutputDirectory("scenarios/coimbraRegion/debug_test");
